...
data = read.table("zscore.skin.all.tsv",sep="\t",header=TRUE,row.names = 1,check.names=F)
...
data = read.table("zscore.blood.all.tsv",sep="\t",header=TRUE,row.names = 1,check.names=F)
bk<-c(seq(min(data),0,length.out = 50),seq(0.01,max(data),length.out = 50))
